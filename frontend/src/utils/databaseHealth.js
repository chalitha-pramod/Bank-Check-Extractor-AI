...
export const getDatabaseDiagnostics = async () => {
  ...
  try {
    // Test API endpoints
    const endpoints = [
      'https://bank-check-extractor-ai-backend.vercel.app/api/health',
      'https://bank-check-extractor-ai-backend.vercel.app/api/test-db',
      'https://bank-check-extractor-ai-backend.vercel.app/api/checks'
    ];
    ...
    if (token) {
      try {
        const authResponse = await fetch('https://bank-check-extractor-ai-backend.vi.vercel.app/api/checks', {
          headers: {
